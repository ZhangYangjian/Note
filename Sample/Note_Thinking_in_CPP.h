...
/* 第十章 名字控制
	@10.1 来自C语言中的静态元素
		* static 两种含义：1、数据在静态数据区存储。2、对一个特定的编译单位来说是局部的，控制名字的可见性。
		* static变量默认初始化的0赋值只对内建类型有效，自定义类型必须用构造函数初始化。
		* abort() 推迟程序，静态对象不执行析构函数。
		* 全局变量和普通函数都有外部连接。
		* 文件作用域内，一个被声明为static的对象或函数的名字只局部于该单元。
		* 所有的全局对象都隐含为静态存储的。
		* 如果局部变量声明为extern，这意味着某处已经存在一个存储区。
	
	@10.2 名字空间
		* 把一个局部名字放在一个未命名的名字空间中，不需要加上static说明就可以让它们作内部连接。

	@10.3 C++ 中的静态成员
		* 类对象的常量和非常量静态数组的初始化必须以相同的方式执行，他们遵守电信的静态定义语法。
	
	@10.4 静态初始化的相依性
		* 尽量避免互相依赖
		* 相依性的变量尽量放在同一个单元下
		 
	@10.5 替代连接说明 extern "C" {} 指定连接类型
...
/*	第十四章 继承和组合
	* 代码重用机制
	...
	@14.5 
		* 普通基类继承中，派生类中只要定义了与基类同名的函数（不管返回值参数是否相同）会隐藏（重定义）基类的同名（包括重载的）函数。
		* 在基类的成员函数是虚函数的情况，不改变函数参数列表和返回类型重新定义函数称为重写（overriding）。
